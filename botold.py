        ...
        for user in users:
            ...
            if member != None:
                ...
                if user[2] == 0 and user[3] > 0:
                    ...
                    except sqlite3.OperationalEperationalError as e:
                        print(e)
                        await client.send_message(client.get_user_info(128215967070289921), e)
                    if member.nick != None:
                        try:
...
def updateLastJoined(member):
    ...
    c.execute("UPDATE USERS SET LASTJOINED = ? WHERE (ID = ? AND SERVERID = ?)",
              (datetime.datetime.now().ctime(), member.id, member.server.id,))
    conn.commit()
    if g:
        conn.close()
        conn = None
...
async def addMember(member):
    ...
    c.execute("INSERT INTO USERS VALUES (?,?,?,?,?,?,?)",
              (member.id, member.server.id, datetime.datetime.now().ctime(), 0, 0, 0, 0,))
    conn.commit()
    if g:
        conn.close()
        conn = None
